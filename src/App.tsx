...
function makeReading(mode:Mode,cards:DrawCard[],c:any,birth:string){ if(!cards.length)return[]; const names=cards.map(x=>x.title).join(" · ");
 ...
 return[
  {icon:"♡",title:mode==="love"?c.sections.flow:c.sections.summary,body:`선택된 카드(${names})는 지금 상황이 단순한 호불호보다 흐름과 타이밍의 문제에 가깝다는 것을 보여줍니다. 첫 카드는 현재의 기본 에너지, 다음 카드는 상대나 환경의 반응, 이후 카드는 앞으로 열릴 가능성을 말합니다. 빠르게 결론을 내기보다 카드들이 가리키는 속도 차이를 읽는 것이 중요합니다.`},
...
function Deck({c,lang}:{c:any;lang:Lang}){ const [q,setQ]=useState(""); const list=TAROT_DECK.filter(card=>`${card.title.en} ${card.title.ko} ${card.id}`.toLowerCase().includes(q.toLowerCase())).slice(0,78); return <section className="deck"><h2>{c.tabs.deck[0]}</h2><input value={q} onChange={e=>setQ(e.target.value)} placeholder="Search card"/><div className="deckGrid">{list.map(card=>{const t=getTarotText(card,lang,false); return <article key={card.id}><img src={`/cards/${card.id}.svg`}/><b>{t.title}</b><p>{t.aura}</p></article>})}</div></section> }
